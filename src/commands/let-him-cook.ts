...
const chefRoles = ['BURGER FLIPPER', 'BAKER', 'PIT MASTER', 'VEGGIE LOVER', 'FRYER', 'SOUS CHEF', 'DOWN-BAD CHEF']

export const LetHimCook: Command = {
  name: 'let-him-cook',
  description: 'Who\s cookin\'?',
  ...
  run: async (_client: Client, interaction: ChatInputCommandInteraction) => {
    ...
    if (!voiceChannel) {
      await interaction.reply('There\s no one here to cook! Is it...WET in here??');
      return;
    }

    const { members } = voiceChannel;
    const selectedUser = members.random();
    await interaction.reply(`In need of a ${role}! LET <@${selectedUser?.id}> COOK!`);
  }
};